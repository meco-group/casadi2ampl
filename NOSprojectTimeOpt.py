...
Nm_c = 3 #amount of circular obstacles
...
Nm_r = 6 #amount of rectangular obstacles
...
for i in range(Nm_c):    
    obj_pred = [BSpline(Blin, cas.vertcat(P0_obj_c[i][0], V0_obj_c[i][0] * T + P0_obj_c[i][0])),
                BSpline(Blin, cas.vertcat(P0_obj_c[i][1], V0_obj_c[i][1] * T + P0_obj_c[i][1]))] #make prediction for obstacle i    
    con1 =  np.dot(np.c_[ax_c[i] , ay_c[i]],np.c_[obj_pred[0],obj_pred[1]].reshape(2,1)) - b_c[i] - r_obj[i]
    con2 = -np.dot(np.c_[x , y].reshape(1,2),np.c_[ax_c[i] , ay_c[i]].reshape(2,1))  + b_c[i] - veh_r
    con1 = con1.tolist()[0] 
    con2 = con2.tolist()[0]   
    con.extend(con1)
    con.extend(con2)
    con.extend([1 - (ax_c[i] * ax_c[i] + ay_c[i] * ay_c[i])])    
...
for i in range(Nm_r):    
    ...
    con1 =  np.dot(np.c_[ax_r[i] , ay_r[i]],np.r_[x_vert , y_vert]) - b_r[i]  
    con2 = -np.dot(np.c_[ax_r[i] , ay_r[i]],np.c_[x , y].reshape(2,1)) + b_r[i] - veh_r
    con1 = con1.tolist()[0]
    con2 = con2.tolist()[0]   
    con.extend(con1)
    con.extend(con2)
...
t_solve = []
# Do the loop
while np.linalg.norm(p0[:2] - [goal[0], goal[1]]) >= 1e-2: #within 1 cm of goal
    solver_input['x0'] = x0
    solver_input['p'] = p0
